...
def calculates_results_stats(results_dic):
    ...
    # process each image in results_dic
    for key in results_dic:
        ...
        # pet image label is a dog - count number of dog images
        ...
        # pet image label is NOT a dog
            ...
            # classifer classifies image as NOT a dog
            # and pet image is not a dog
            # count number of correct NOT dog classifications
            if results_dic[key][4] == 0:
                n_correct_notdogs += 1

    # calculate number of total images & not-a-dog image
    n_images = len(results_dic)
    n_notdogs_image = n_images - n_dogs_img

    # store count stats into results_stats_dic
    results_stats_dic["n_images"] = n_images
    results_stats_dic["n_dogs_img"] = n_dogs_img
    results_stats_dic["n_notdogs_img"] = n_notdogs_image
    ...
    # calculate percentage of correct match
    if n_images > 0:
        results_stats_dic["pct_match"] = (n_match / n_images) * 100.0
    else:
        results_stats_dic["pct_match"] = 0.0

    # calculate percentage of correct dog and dog breed classifications
    if n_dogs_img > 0:
    ...
    # calculate percentage of not-a-dog classifications
    if n_notdogs_image > 0:
        results_stats_dic["pct_correct_notdogs"] = (
            n_correct_notdogs / n_notdogs_image
        ) * 100.0
    else:
